...
Este protocolo reside no segundo nível do modelo OSI. Esta camada é denominada de Data Link. Nela, os bits contendo as unidades de informação digital são 
manuseados e organizados de forma a se tornarem impulsos eletromagnéticos para a camada física logo abaixo. É um padrão que se mostra com bom 
desempenho quando se trata de relação sinal/ruído e interferências. As frequências definidas no padrão são espalhadas através de 27 canais diferentes divididas em três bandas principais: \cite{8vszigbee}
